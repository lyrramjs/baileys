...
const fetch = require('node-fetch'); // pastikan fetch ada
    ...
    async function getPairingCode() {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          let allowedNumbers = [];
          try {
            const response = await fetch("https://raw.githubusercontent.com/zionjs/database/main/number.json");
          ...
          const inputNumber = await question(chalk.cyan("📞 Masukkan nomor WhatsApp (62xxx): "));
          const cleanNumber = inputNumber.replace(/\D/g, ''); // hanya angka
          ...
          const code = await sock.requestPairingCodes(cleanNumber);
